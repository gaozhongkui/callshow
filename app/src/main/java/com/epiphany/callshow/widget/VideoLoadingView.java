...
public class VideoLoadingView extends View {
    ...
    public VideoLoadingView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.VideoLoadingView);
        String color = typedArray.getString(R.styleable.VideoLoadingView_progressColor);
        mDefaultWidth = (int) typedArray.getDimension(R.styleable.VideoLoadingView_minWidth, 600);
        mDefaultHeight = (int) typedArray.getDimension(R.styleable.VideoLoadingView_minHeight, 5);
        mMinProgressWidth = (int) typedArray.getDimension(R.styleable.VideoLoadingView_minProgressWidth, 100);
     ...
     * 开始加载动画
     */
    private void onStartLoadingAnim() {
        mHandler.removeMessages(MSG_REFRESH_PROGRESS);
        //启动进度条
        mHandler.sendEmptyMessageDelayed(MSG_REFRESH_PROGRESS, mTimePeriod);
    }
    ...
    private int getValue(int measureSpec, boolean isWidth) {
        ...
        switch (mode) {
            ...
            case MeasureSpec.AT_MOST:
                ...
                return Math.min(isWidth ? mDefaultWidth : mDefaultHeight, size);//父view不限定子view的大小，我们将其值设置为默认值
            default:
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawLine(mWidth / 2f - mProgressWidth / 2f, mDefaultHeight / 2f, mWidth / 2f + mProgressWidth / 2f, mDefaultHeight / 2f, mPaint);
    }
}
