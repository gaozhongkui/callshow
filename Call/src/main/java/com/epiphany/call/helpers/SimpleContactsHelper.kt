package com.epiphany.call.helpers
...
import com.epiphany.call.models.SimpleContact
...
class SimpleContactsHelper(val context: Context) {
    ...
    private fun getContactPhoneNumbers(favoritesOnly: Boolean): ArrayList<SimpleContact> {
        ...
        context.queryCursor(uri, projection, selection) { cursor ->
            val phoneNumber = cursor.getStringValue(CommonDataKinds.Phone.NORMALIZED_NUMBER)
                ?: cursor.getStringValue(CommonDataKinds.Phone.NUMBER)?.normalizePhoneNumber()
                ?: return@queryCursor
    ...
    fun getContactEvents(getBirthdays: Boolean): SparseArray<ArrayList<String>> {
        ...
        val selection =
            "${CommonDataKinds.Event.MIMETYPE} = ? AND ${CommonDataKinds.Event.TYPE} = ?"
        val requiredType =
            if (getBirthdays) CommonDataKinds.Event.TYPE_BIRTHDAY.toString() else CommonDataKinds.Event.TYPE_ANNIVERSARY.toString()
